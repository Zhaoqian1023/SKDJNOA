...
<p style="padding: 25px 0px 10px; position: relative;" ><h2>管理员登录</h2></p>
        <P style="padding: 20px 0px 10px; position: relative;">
            <SPAN class="u_logo"></SPAN> <INPUT class="ipt" id="uname"
                name="uname" type="text" value="" placeholder="请输入管理员账号">
        ...
        <DIV
            ...
            <span  id="tiaozhuan"
                style="float: left;margin-left: 10px"><a href="../admin/login.jsp"><strong>点此进入教师登录页面</strong></a></span>
                <SPAN
                style="margin-left: 50px;"> <input id="loginBtn" type="submit"
                value="登录"
	...
	<script type="text/javascript">
		$("#loginBtn").click(
				function() {
					...
					var loginInfo = "mid=" + uid + "&passWord=" + passWord;
					if (uid == "" || passWord == "") {
						showMessage(3);
					} else {
						$.ajax({
							type : "post",
							url : "Manager_login.action",
							data : loginInfo,
							/* dataType:"json", */
							async : false,
							success : function(result) {
								...
								if (data.name == "login"
									...
									window.location.href = "index_manager.jsp";
								} else {
									showMessage(0);
								}
							},
